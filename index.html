    ...
    <div class="flex justify-end space-x-3">
        ...
        <button type="submit" class="btn-primary">
          ...
          <section id="transfer-section" class="solana-card p-6 hidden">
            <h2 class="text-xl font-semibold mb-6">Transfer Saldo</h2>
            ...
            <form id="p2p-transfer-form" class="space-y-4 mb-6">
              ...
              <div>
                <label class="block text-sm mb-2">Jumlah (Rp)</label>
                <input type="number" id="transfer-amount" class="input-field" placeholder="50000" required min="1000">
              </div>
              <div>
                <label class="block text-sm mb-2">Pesan</label>
                <input type="text" id="transfer-note" class="input-field" placeholder="Untuk pembayaran..." maxlength="100">
              </div>
              <div>
                <label class="block text-sm mb-2">PIN Transaksi</label>
                <input type="password" id="transfer-pin" class="input-field" placeholder="••••" maxlength="4" required>
              </div>
              <div id="transfer-error-message" class="text-red-400 text-sm hidden"></div>
              <div class="flex justify-end">
                <button type="submit" class="btn-primary">Kirim Transfer</button>
              </div>
            </form>
          
            <h3 class="text-lg font-medium mb-4">Riwayat Transfer Saya</h3>
            <div id="p2p-transactions-list" class="space-y-4 max-h-[400px] overflow-y-auto">
              <div class="text-center py-10">
                <div class="loading mx-auto"></div>
...
function initAuthListeners() {
  ...
  try {
    ...
    showError("Gagal menghapus transaksi: " + error.message);
     ...
     // ================= FUNGSI TRANSFER P2P =================
     // Global variable for admin fee (can be fetched from Firestore for dynamic fee)
const ADMIN_FEE_PERCENTAGE = 0.01; // 1% admin fee

function initTransferListeners() {
  const p2pTransferForm = document.getElementById('p2p-transfer-form');
  ...
  p2pTransferForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    transferErrorMessage.classList.add('hidden'); // Hide previous errors
    ...
    if (pin.length !== 4) { // Basic PIN length check
      showError('PIN transaksi harus 4 digit.');
      return;
    }
    if (recipientEmail === currentUser.email) {
        showError('Anda tidak bisa transfer ke akun Anda sendiri.');
        return;
    }

    try {
      // 1. Verifikasi PIN (Contoh: PIN sederhana, sebaiknya di-hash dan diverifikasi di backend)
      const userDoc = await usersRef.doc(currentUser.uid).get();
      if (!userDoc.exists) {
      ...
      const userData = userDoc.data();
      if (userData.transactionPin !== pin) { // Comparing plain text PIN (INSECURE FOR PRODUCTION)
          showError('PIN transaksi salah.');
          return;
      }
      // RECOMMENDED: For production, send PIN to a Cloud Function to verify securely and then initiate transfer.
      ...
      // 4. Periksa Saldo Pengirim
      if (userData.totalBalance < totalAmountToSend) {
        showError(`Saldo tidak mencukupi. Anda perlu Rp ${formatRupiah(totalAmountToSend)} (termasuk biaya admin).`);
        return;
      }

      // 5. Jalankan Transaksi Firestore (Atomic Operation)
      await db.runTransaction(async (transaction) => {
        ...
        const currentSenderBalance = senderSnap.data().totalBalance || 0;
        const currentRecipientBalance = recipientSnap.data().totalBalance || 0;

        // Double check balance (important for concurrent transactions)
        if (currentSenderBalance < totalAmountToSend) {
          throw new Error('Saldo tidak mencukupi (diperiksa ulang).');
        }

        // Update balances
        transaction.update(senderDocRef, {
          totalBalance: currentSenderBalance - totalAmountToSend,
          expense: (senderSnap.data().expense || 0) + totalAmountToSend // Track as expense for sender
        });
        transaction.update(recipientDocRef, {
          totalBalance: currentRecipientBalance + amount, // Recipient receives net amount
          income: (recipientSnap.data().income || 0) + amount // Track as income for recipient
        });

        // Catat Transaksi Pengirim (Keluar)
        const senderTransactionRef = transactionsRef.doc();
        transaction.set(senderTransactionRef, {
          userId: currentUser.uid,
          type: 'p2p_transfer_out',
          amount: totalAmountToSend,
          description: `Transfer ke ${recipientData.name || recipientEmail}: ${note}`,
        ...
        const recipientTransactionRef = transactionsRef.doc();
        transaction.set(recipientTransactionRef, {
          userId: recipientId,
          type: 'p2p_transfer_in',
          amount: amount, // Recipient sees net amount received
          description: `Terima transfer dari ${userData.name || currentUser.email}: ${note}`,
          timestamp: firebase.firestore.FieldValue.serverTimestamp(),
          senderId: currentUser.uid,
          senderEmail: currentUser.email,
          adminFee: 0, // Admin fee is on sender's side
          netAmount: amount
        });
        ...
        notificationsRef.add({
            userId: currentUser.uid,
            message: `Transfer sebesar ${formatRupiah(amount)} ke ${recipientData.name || recipientEmail} berhasil. Biaya admin: ${formatRupiah(adminFee)}.`,
            type: 'transfer_sent',
            read: false,
            timestamp: firebase.firestore.FieldValue.serverTimestamp(),
        ...
        notificationsRef.add({
            userId: recipientId,
            message: `Anda menerima transfer sebesar ${formatRupiah(amount)} dari ${userData.name || currentUser.email}.`,
      ...
      p2pTransferForm.reset(); // Clear form
      loadUserData(); // Update dashboard balances
      loadP2PTransactions(); // Refresh transaction list
...
// Fungsi untuk memuat dan menampilkan riwayat transfer P2P
async function loadP2PTransactions() {
  ...
  try {
    const querySnapshot = await transactionsRef
      ...
      .limit(20) // Limit to latest 20 transfers
      .get();
    ...
    querySnapshot.forEach(doc => {
      ...
      const amountColor = transaction.type === 'p2p_transfer_in' ? 'text-green-400' : 'text-red-400';
...
// Panggil initTransferListeners() di DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
  // ... (existing initializations)
  initTransferListeners(); // NEW
  // ...
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    function initNavigationListeners() {
      ...
      document.getElementById('show-transfer').addEventListener('click', () => { // NEW
        hideAllSections();
        document.getElementById('transfer-section').classList.remove('hidden');
        loadP2PTransactions(); // Load transactions when section is shown
      });
